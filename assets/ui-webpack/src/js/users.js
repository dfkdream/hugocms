...
i18n().then(t => {
        ...
        return new Promise((resolve, reject) => {
            ...
            fragment.getElementById("modal-form").onsubmit = (e) => {
                ...
                fetch("/admin/api/users", {
                    ...
                    .then(resp => {
                        if (!resp.ok) {
                            if (resp.status === 409) popups.alert(document.body, t("error"), t("errIdConflict"));
                            else popups.alert(document.body, t("error"), `Error ${resp.status}`);
                        } else {
                            closePopup();
                            resolve(true);
                        }
    ...
    const showEditUserPopup = (uid) => {
        return new Promise((resolve, reject) => {
            ...
            fragment.getElementById("title").innerText = t("editUser");
        ...
        fetch("/admin/api/users")
            ...
            .then(users => {
                    ...
                    users.forEach(user => {
                        ...
                        editBtn.dataset.tooltip = t("tooltipEdit");
                        ...
                        deleteBtn.dataset.tooltip = t("tooltipDelete");

                        deleteBtn.onclick = () => {
                            popups.confirm(document.body, t("confirmDelete"), `Delete User ${user.id}?`)
                                .then(ok => {
                                    if (ok) {
                                        fetch("/admin/api/user/" + user.id, {method: "DELETE"})
                                            .then(resp => {
                                                if (resp.ok) loadUsers();
                                                else popups.alert(document.body, t("error"), `${resp.status} ${resp.statusText}`)
                                            })
                                    }
                                })
                        };
